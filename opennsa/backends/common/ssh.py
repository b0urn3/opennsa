...
class SSHChannel(channel.SSHChannel):
    ...
    def __init__(self, localWindow=0, localMaxPacket=0, remoteWindow=0, remoteMaxPacket=0, conn=None, data=None,
                 avatar=None):
        channel.SSHChannel.__init__(self, localWindow, localMaxPacket, remoteWindow, remoteMaxPacket, conn, data,
                                    avatar)
    ...
    def getSSHConnection(self):
        ...
        def gotTCPConnection(proto):
            ...
            if self.public_key_path and self.private_key_path:
                proto.requestService(
                    KeyUserAuthClient(self.username, ssh_connection, self.public_key_path, self.private_key_path))
            elif self.password:
